...
function humanizeClerkError(
  ...
  const map: Record<string, string> = {
    ...
    form_not_found_lenient: 'Account not found. Check your email or sign in.',
  };
    ...
    try {
      ...
      if (pwError) {
        setError(
          humanizeClerkError(
            pwError.code,
            pwError.longMessage ?? pwError.message ?? 'Sign up failed.',
          ),
        );
      ...
      if (signUp.status === 'missing_requirements') {
        // Email verification needed — send the OTP code.
      ...
      setError(
        humanizeClerkError(
          first?.code,
          first?.longMessage ?? first?.message ?? 'Sign up failed.',
        ),
      );
      setIsSubmitting(false);
    }
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    try {
      ...
      setError(
        humanizeClerkError(
          first?.code,
          first?.message ?? 'Verification failed.',
        ),
      );
      setIsSubmitting(false);
    }
    ...
    try {
      ...
      if (ssoError) {
        setError(
          humanizeClerkError(
            ssoError.code,
            `${strategy === 'oauth_google' ? 'Google' : 'GitHub'} sign-up failed.`,
          ),
        );
      ...
      const first = clerkErr.errors?.[0];
      setError(humanizeClerkError(first?.code, 'OAuth sign-up failed.'));
      setOauthLoading(null);
    }
  }
      ...
      <div className="space-y-5">
        <div className="flex items-start gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-sm text-foreground">
          ...
          <span>
            ...
            <strong className="font-semibold">{email}</strong>. Enter it below
        ...
        <form onSubmit={handleVerify} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input
              ...
              placeholder="123456"
              className="h-12 rounded-xl border-border/70 bg-background/80 px-3.5 text-base tracking-[0.4em] text-foreground backdrop-blur-md transition-colors placeholder:text-muted-foreground placeholder:tracking-normal focus-visible:border-primary/60 focus-visible:ring-3 focus-visible:ring-primary/15"
            />
          </div>
